#!/usr/bin/env python3
"""
Desktop Pet Hybrid Rendering Engine (Orange Tabby Cat)
=========================================================
CONFIG: PET_NAME / PET_NAME_ASCII / BARK_TEXT / EAT_TEXT + ANIMS dict
Core principles (same as Deskpet competitor):
...
# ═══ CONFIG (Orange Tabby Cat Pet) ═══
PET_NAME = 'Orange Cat'                   # Display name
PET_NAME_ASCII = 'OrangeCat'              # ASCII name (window title / mutex)
BARK_TEXT, EAT_TEXT = 'Meow!', 'Yummy!'   # Bubble text
...
ANIMS = {
    ...
    'meow':      ('meow',      121, 42,  True,  0),
    'purr':      ('purr',      121, 42,  True,  0),
    'groom':     ('groom',     121, 42,  True,  0),
    'sleep':     ('sleep',     121, 42,  True,  0),
    'sit':       ('sit',       121, 42,  True,  0),
    'scratch':   ('scratch',   121, 42,  True,  0),
...
CANVAS_H = 320        # Window height (vertical baseline for all states)
CANVAS_W = 320        # Window width default for upright states; wide states override via sprite_meta.canvas_w
...
class _StateLoadThread(QThread):
    ...
    def run(self):
        try:
            ...
            ref_h = state_meta.get('ref_height', max_dim)
            # v119: Uniform height scale — wide states expand canvas_w instead of shrinking cat
            sc = draw / float(ref_h) if ref_h > 0 else 1.0
...
class SpriteBank:
    ...
    TIGHT = {'idle', 'run', 'eat', 'meow', 'purr', 'groom', 'sleep', 'sit', 'scratch', 'happy',
             'roll', 'dance', 'stretch', 'beg', 'bath', 'surprised',
             'play_dead', 'wave', 'type', 'walk'}
    ...
    #   These states (walk/run/meow/sit/sleep/eat/lick) are the most common AI auto-actions.
    #   Low-frequency performance states (dance/beg/bath/happy/stretch/play_dead/surprised/wave/type/roll/purr/groom) stay lazy.
    LAZY = {'idle', 'happy', 'stretch', 'play_dead', 'surprised',
            'dance', 'beg', 'bath', 'wave', 'type', 'roll',
            'run', 'eat', 'meow', 'purr', 'groom', 'sleep', 'sit', 'scratch', 'walk'}  # v99: ALL non-idle states lazy — startup loads only idle
    ...
    def ensure_state(self, state):
        """v114: Progressive loading — sync 20 frames for instant display, then async-load remaining.
        ...
        v119: Fixed check — only skip if frames are FULLY loaded (count matches ANIMS), not just non-empty."""
        owner = self.alias.get(state, state)
        _count = ANIMS.get(state, ANIMS.get(owner, (None, 0)))[1] if owner in ANIMS or state in ANIMS else 0
        _loaded = len(self.frames.get(owner, []))
        if owner not in self.LAZY or _loaded >= _count:
            return
    ...
    def unload_idle_lazy(self, active_state, keep=1):
        ...
        for st in list(order):
            ...
            if st not in self.LAZY:
                continue
            # v119: Never unload idle (permanent resident)
            if st == 'idle':
                continue
    ...
    def _build_state(self, state, frame_limit=None, max_frames=None):
        ...
        if state_meta and tight:
            ...
            ref_h = (state_meta or {}).get('ref_height', max_dim) if state_meta else max_dim
            # v119: Uniform height scale — wide states expand canvas_w instead of shrinking cat
            sc = draw / float(ref_h) if ref_h > 0 else 1.0
            imgs = []
            for img in raw_imgs:
    ...
    def _center_frames(self, imgs, shift=None):
        ...
        Cause: walk frame content hugs left edge (left margin=0), centroid offset -44px, after mirror cat head at window edge;
        and inter-frame centroid drift causes left-right wobble. Use sequence-average centroid for unified shift  -- 
        both centers cat in window and eliminates inter-frame jitter. v25: draw at current zoom size.
    ...
    def _trim_pixmaps(self, state, idx):
        ...
        if pm_m_cache:
            n_m = len(pm_m_cache)
            half_m = self.PM_WINDOW // 2
            lo_m = max(0, idx - half_m)
            hi_m = min(n_m, idx + half_m + 1)
            for j in range(n_m):
                if j < lo_m or j >= hi_m:
...
class PetWindow(QWidget):
    def __init__(self):
        ...
        self.zoom = load_zoom()
        self.canvas_w = CANVAS_W  # v119: per-state canvas width (updated on state change)
        self._pending_canvas_w = CANVAS_W  # v120: target canvas_w (deferred for shrink)
        self.setFixedSize(int(self.canvas_w * self.zoom), int(CANVAS_H * self.zoom))
        ...
        # Sound effects (parent=self prevents QSoundEffect GC; preload eliminates first-play delay)
        ...
        self.floor_y = sg.bottom() - int(CANVAS_H * self.zoom) - 45
        self.move(sg.center().x() - int(self.canvas_w * self.zoom) // 2, self.floor_y)
    ...
    def set_zoom(self, new_zoom):
        ...
        self.zoom = new_zoom
        save_zoom(new_zoom)
        new_w = int(self.canvas_w * new_zoom)
        new_h = int(CANVAS_H * new_zoom)
        old_w, old_h = self.width(), self.height()
        self.setFixedSize(new_w, new_h)
        sg = QApplication.primaryScreen().geometry()
        self.floor_y = sg.bottom() - new_h - 45
        # Zoom anchored at window center, clamped within screen
        nx = self.x() + old_w // 2 - new_w // 2
        if self.mode == 'desktop':
            # drag-drop fixed mode: keep current y position, do not snap back to bottom
            ny = self.y() + old_h - new_h
        else:
            ny = min(self.y() + old_h - new_h, self.floor_y)
        nx = max(0, min(nx, sg.right() - new_w))
        ny = max(0, min(ny, sg.bottom() - new_h))
        self.move(nx, ny)
    ...
    def _on_zoom_loaded(self):
        ...
        if t._seq == getattr(self, '_load_seq', 0) and t.result is not None:
            ...
            # causing: (1) memory never freed, (2) draw_rect mismatch → big cat overflows small canvas.
    ...
    #  --  --  --  --  -- ─ State switching  --  --  --  --  -- ─
    def _resize_for_state(self, state):
        """v119→v120: Resize window to match state's canvas_w.
        Wide states (walk/run/eat/sleep etc.) need a wider window so the cat
        isn't clipped. The extra horizontal space is transparent (WA_TranslucentBackground)
        so only the cat is visible to the user.
        v120-fix: Defer canvas_w shrink — when switching from wide→narrow, don't
        shrink immediately because the old frame may still be shown. Instead, set
        _pending_canvas_w and apply the shrink once the new state's frames are loaded."""
        owner = self.bank.alias.get(state, state) if hasattr(self, 'bank') and self.bank else state
        meta = self.bank._sprite_meta if hasattr(self, 'bank') and self.bank else None
        state_meta = meta.get(owner) if meta else None
        new_cw = state_meta.get('canvas_w', CANVAS_W) if state_meta else CANVAS_W
        old_cw = getattr(self, 'canvas_w', CANVAS_W)
        if new_cw == old_cw:
            return  # no resize needed
        if new_cw > old_cw:
            # Growing: safe to do immediately (old frame fits within wider window)
            self.canvas_w = new_cw
            self._pending_canvas_w = new_cw
            new_w = int(new_cw * self.zoom)
            new_h = int(CANVAS_H * self.zoom)
            old_w = self.width()
            self.setFixedSize(new_w, new_h)
            dx_pixels = (new_w - old_w) // 2
            nx = self.x() - dx_pixels
            sg = QApplication.primaryScreen().geometry()
            nx = max(0, min(nx, sg.right() - new_w))
            self.move(nx, self.y())
            ...
            # Shrinking: defer until new state frames are loaded
            self._pending_canvas_w = new_cw

    def _commit_pending_resize(self):
        """v120: Apply deferred canvas_w shrink now that new state frames are ready."""
        pending = getattr(self, '_pending_canvas_w', None)
        if pending is None or pending == getattr(self, 'canvas_w', CANVAS_W):
            return
        new_cw = pending
        self.canvas_w = new_cw
        new_w = int(new_cw * self.zoom)
    ...
    def set_state(self, s, duration=None):
        ...
        self.state = s
        # v119: Resize window to match state's canvas_w (wide states need wider window)
        self._resize_for_state(s)
        self.state_started = time.perf_counter()
        if not same_move:
            self.anim_elapsed = 0.0
            self.frame_idx = 0
        ...
        # Sound effect: play on state change
        if not same_move and hasattr(self, '_sound') and self._sound:
            self._sound.play(s)
        ...
        if s == 'meow':
            self.particles.emit(ParticleSystem.BUBBLE, self.canvas_w / 2, 60, 1, text=BARK_TEXT)
        elif s == 'happy':
            self.particles.emit(ParticleSystem.HEART, self.canvas_w / 2, 90, 4)
        elif s == 'eat':
            self.say(EAT_TEXT)
    ...
    def say(self, text):
        self.particles.emit(ParticleSystem.BUBBLE, self.canvas_w / 2, 55, 1, text=text)
    ...
    def _integrate_vel(self, dt, target, accel, decel):
        ...
        if step:
            ...
            # old tease mode chased mouse without bounds -> cat ran off screen, head/tail clipped by screen edge.
            sg = QApplication.primaryScreen().geometry()
    ...
    def turn_scale(self):
        ...
        no X-axis scaling at all, with 0.22s fast turn, looks like cat turning crisply"""
        return 1.0
    ...
    def game_loop(self):
        ...
        if not skip_ai_physics:
            ...
            if impact > 200:
                self.particles.emit(ParticleSystem.DUST, self.canvas_w / 2, CANVAS_H - GROUND_PAD, 4)
    ...
    def update_ai(self, dt):
        ...
        # v113-fix: loop=False states (happy/roll/stretch/play_dead) that finished
        ...
        if self.mode == 'tease':
            ...
            if dist < 150 and random.random() < dt * 2.5:
                self.particles.emit(ParticleSystem.HEART, self.canvas_w / 2, 80, 1)
            return
        ...
        if st == 'potty':
            if st_time > 3.0:
                self.set_state('idle')
                self.particles.emit(ParticleSystem.SPARKLE, self.canvas_w / 2, 100, 5)
        ...
        if self.mode == 'taskbar':
            if r < 0.42:
                ...
                self.set_state('meow', duration=5.08)
            elif r < 0.76:
                self.set_state('scratch', duration=5.08)
            elif r < 0.84:
    ...
    def emit_state_particles(self, dt):
        ...
        if st == 'sleep' and random.random() < dt * 0.8:
            self.particles.emit(ParticleSystem.ZZZ, self.canvas_w / 2 + 45, 120, 1)
        elif st == 'potty' and random.random() < dt * 2.5:
            self.particles.emit(ParticleSystem.STINK, self.canvas_w / 2 + 30, CANVAS_H - 60, 1)
        elif st == 'eat' and random.random() < dt * 4:
            self.particles.emit(ParticleSystem.CRUMB, self.canvas_w / 2 + 35, CANVAS_H - 70, 1)
        elif st == 'bath' and random.random() < dt * 3:
            self.particles.emit(ParticleSystem.SPARKLE, self.canvas_w / 2, 100, 1)
    ...
    def paintEvent(self, event):
        ...
        cw = getattr(self, 'canvas_w', CANVAS_W)  # v119: per-state canvas width
        dx = cw / 2
        dy = CANVAS_H - GROUND_PAD  # Foot anchor point
        ...
        if st == 'idle' or st == 'sit':
            ...
            pass
        elif st in ('bath', 'surprised', 'stretch', 'beg', 'scratch', 'sit', 'sleep'):
            pass
        ...
        # v25 (P9): unified zoom -- all drawing logic keeps CANVAS_H=320 logical coordinate system
        painter.scale(self.zoom, self.zoom)
        # v116→v119: Clip to per-state canvas size (wide states have wider canvas_w)
        # v120-fix: Use the LARGER of current and target canvas_w for clip.
        # When switching from wide→narrow state, the old frame (_last_frame) may still
        # be displayed briefly while new state loads. If we clip to the new narrow canvas_w
        # immediately, the old wide frame gets truncated on screen.
        # Solution: keep clip wide enough for whichever frame is actually shown.
        clip_cw = max(getattr(self, 'canvas_w', CANVAS_W),
                      getattr(self, '_pending_canvas_w', CANVAS_W))
        painter.setClipRect(QRectF(0, 0, clip_cw, CANVAS_H))
        ...
        if pm.isNull():
            ...
            self._last_frame = pm
            # v120: New state frame is ready — apply deferred canvas_w shrink if pending
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton and self.dragging:
            ...
            if self.mode == 'desktop':
                ...
                self.particles.emit(ParticleSystem.HEART, self.canvas_w / 2, 90, 2)
                return
    ...
    def mouseDoubleClickEvent(self, event):
        trick = random.choice(['happy', 'roll', 'dance', 'meow'])
    ...
    def contextMenuEvent(self, event):
        ...
        # kiss removed (hand interaction, darkblue BG issue)
        trick_menu.add_item('type', 'Type')
        trick_menu.add_item('play_dead', 'Play Dead')
        trick_menu.add_item('sit', 'Sit')
        ...
        menu.add_sep()
        # Sound submenu
        sound_menu = RoundedMenu(self)
        sound_menu.add_item('sound_up', '🔊 Volume Up')
        sound_menu.add_item('sound_down', '🔉 Volume Down')
        mute_label = '🔇 Unmute' if self._sound.muted else '🔇 Mute'
        sound_menu.add_item('sound_mute', mute_label)
        menu.add_sub('🔔 Sound', sound_menu)
        menu.add_sep()
        ...
        if action == 'tease':
            ...
            self.set_state('purr', duration=10.16)
        elif action == 'happy':
            ...
            self.set_state('groom', duration=10.16)
        elif action == 'type':
            self.set_state('type', duration=10.16)
        elif action == 'play_dead':
            self.set_state('play_dead', duration=10.16)
        elif action == 'sit':
            self.set_state('sit', duration=10.16)
        elif action == 'walk':
            # Perform walk: set direction and duration, make the cat walk
            self.walk_dir = random.choice([-1, 1])
            self._start_turn(self.walk_dir)
            if self.turn_phase is None:
                self.facing = self.walk_dir
            ...
            self.say('🔇 Muted' if muted else '🔔 Unmuted')
        elif action == 'sleep':
            self.set_state('sleep')
        elif action == 'stats':
            self.say(f'Full{int(self.fullness)} Happy{int(self.happiness)} '
                      f' Energy{int(self.energy)}')
            self.particles.emit(ParticleSystem.SPARKLE, self.canvas_w / 2, 70, 4)
        elif action == 'quit':
            QApplication.quit()
...
def main():
    ...
    if not _acquire_single_instance():
        ...
        # fixes "new EXE always activates zombie old window, user sees frozen pet" dead loop.
        hwnd = _find_existing_window()
